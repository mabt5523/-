<template>
  <view class="page">
    <view class="header">
      <text class="title">第 43 关：学会「或」OR 🧠</text>
      <text class="tip">学习点：复合条件（边界 或 墙 或 红灯）</text>
    </view>
    ...
    <view class="panel">
      <view class="panel-title">
        指令序列（点可删除）
        <text class="hint">提示：用「如果前方不可走就右转」更聪明</text>
      </view>
      ...
      <view class="rule">
        ...
        <text>
          红灯❗不能踩；撞墙🧱或越界会停止。
          「🧠 前方不可走就右转」= 只要（越界 或 墙 或 红灯）任意一个成立，就右转。
        </text>
      </view>

      <view class="hintbox">
        <text class="hint-title">本关知识点：</text>
        <text>OR（或）= 条件A 或 条件B：只要有一个为真，就执行。</text>
      </view>
    </view>
    ...
    <view v-if="success" class="success-popup" @touchmove.stop.prevent>
      <view class="success-content">
        <text class="success-text">🎉 通关成功！你会用 OR 判断啦</text>
...
export default {
  data() {
    return {
      ...
      // ✅ 地图：让玩家必须靠 OR 判断避开“墙/红灯/边界”
      // 走廊 + 拐弯 + 红灯，逼你用“前方不可走就右转”做自适应
      blocks: [
        { r: 3, c: 1 }, { r: 3, c: 2 }, { r: 3, c: 3 }, // 中间挡墙
        { r: 1, c: 0 }, { r: 1, c: 1 }, { r: 1, c: 2 }  // 顶部左侧挡墙
      ],
      dangers: [
        { r: 4, c: 3 }, // 底边红灯：逼你不能一路直走到底
        { r: 2, c: 4 }  // 右侧红灯：逼你提前拐弯
      ],
      ...
      for (let i = 0; i < this.commands.length; i++) {
        ...
        if (cmd.type === 'turnLeft') {
          ...
          if (this.isDanger(next)) {
            uni.showToast({ title: '踩到红灯❗了！', icon: 'none' })
            break
          }
          this.robot = next
          await this.sleep(360)
