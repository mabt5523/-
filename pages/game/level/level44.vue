<template>
  <view class="page">
    <view class="header">
      <text class="title">第 44 关：循环分支 🤖🧠</text>
      <text class="tip">学习点：While 循环 + If/Else 分支（循环里做判断）</text>
    </view>
    ...
    <view class="panel">
      <view class="panel-title">
        指令序列（点可删除）
        <text class="hint">提示：这一关用「循环分支」一条就能过</text>
      </view>
      ...
      <view class="cmd-list">
        <template v-for="(cmd, index) in commands" :key="index">
          <!-- 循环分支大积木 -->
          <view
            v-if="cmd.type === 'whileNotSuccessIfBlockedTurnRightElseMove'"
            class="cmd loopif-card"
            @click="removeCommand(index)"
          >
            <view class="loop-head">
              <text class="loop-badge">🔁🧠 循环分支：直到到达⭐</text>
              <text class="cmd-close">×</text>
            </view>
            <view class="loop-body">
              <view class="sub-cmd">
                <text>while(没到⭐){ if(前方不可走)↻右转 else ➕前进 }</text>
              </view>
            </view>
          </view>
          ...
          <view v-else class="cmd" @click="removeCommand(index)">
            <view class="cmd-left">
              <text class="badge" v-if="cmd.type === 'untilBlocked'">🔁</text>
              <text class="badge or" v-if="cmd.type === 'ifFrontBlockedOrDangerTurnRight'">OR</text>
      ...
      <view class="buttons">
        ...
        <!-- 上一关 OR（可选） -->
        <view class="btn orbtn" @click="add('ifFrontBlockedOrDangerTurnRight')" :class="{ disabled: isFull || isMoving }">
          🧠 前方不可走就右转
        </view>

        <!-- 上一关 Until（可选） -->
        <view class="btn until" @click="addUntil" :class="{ disabled: isFull || isMoving }">
          🔁 直到不能走
        </view>

        <!-- ✅ 本关核心：循环分支 -->
        <view class="btn loopif" @click="add('whileNotSuccessIfBlockedTurnRightElseMove')" :class="{ disabled: isFull || isMoving }">
          🔁🧠 循环分支到⭐
        </view>
      ...
      <view class="rule">
        ...
        <text>
          红灯❗不能踩；撞墙🧱或越界会停止。
          「循环分支到⭐」会一直循环：如果（越界 或 墙 或 红灯）任意一个成立就右转，否则前进，直到到达⭐或触发安全上限。
        </text>
      </view>

      <view class="hintbox">
        <text class="hint-title">本关知识点：</text>
        <text>循环分支 = while 循环里写 if/else：每一轮先判断，再决定做什么。</text>
      </view>
    </view>
    ...
    <view v-if="success" class="success-popup" @touchmove.stop.prevent>
      <view class="success-content">
        <text class="success-text">🎉 通关成功！你掌握「循环 + 分支」啦</text>
...
export default {
  data() {
    return {
      ...
      // ✅ 地图（小迷宫）：逼你“循环里判断”
      // 思路：直走会遇到红灯/墙/边界，必须在循环里不断判断并右转，最终绕到终点
      blocks: [
        { r: 1, c: 0 }, { r: 1, c: 1 }, { r: 1, c: 2 }  // 顶部左侧挡墙（别挡到c=4）
      ],
      dangers: [
        { r: 4, c: 3 } // 底边红灯：走到(4,2)时前方不可走 -> 触发右转逻辑
      ],
  ...
  methods: {
    ...
    cmdText(cmd) {
      ...
      if (cmd.type === 'ifFrontBlockedOrDangerTurnRight') return '如果前方不可走：右转'
      if (cmd.type === 'whileNotSuccessIfBlockedTurnRightElseMove') return '循环分支到⭐'
      return cmd.type
    },
    ...
    // ✅ 本关核心：while + if/else
    async execLoopBranchUntilSuccess() {
      const SAFE_LIMIT = 80
      let guard = 0

      while (!this.checkSuccess() && guard < SAFE_LIMIT) {
        if (this.frontNotWalkable()) {
          this.turnRight()
          await this.sleep(220)
        } else {
          this.robot = this.getNextCell()
    ...
    // ============ until（可选，沿用 13 的“运行时循环”写法） ============
    async execUntilNextAction(nextAction) {
      const SAFE_LIMIT = 40
      let guard = 0
      ...
      while (guard < SAFE_LIMIT) {
        if (nextAction.type === 'move') {
          const next = this.getNextCell()
          if (!this.inBounds(next) || this.isBlocked(next) || this.isDanger(next)) break
          this.robot = next
          await this.sleep(340)
        } else if (nextAction.type === 'turnLeft') {
          this.turnLeft()
          await this.sleep(220)
        } else if (nextAction.type === 'turnRight') {
          this.turnRight()
          await this.sleep(220)
        }
        guard++
    ...
    async run() {
      ...
      for (let i = 0; i < this.commands.length; i++) {
        ...
        if (cmd.type === 'turnLeft') {
          ...
          this.turnRight()
          await this.sleep(220)
        } else if (cmd.type === 'ifFrontBlockedOrDangerTurnRight') {
          if (this.frontNotWalkable()) this.turnRight()
          await this.sleep(240)
        } else if (cmd.type === 'untilBlocked') {
          const next = this.commands[i + 1]
          if (next && next.type !== 'untilBlocked') {
            await this.execUntilNextAction(next)
            i += 1
          }
        } else if (cmd.type === 'whileNotSuccessIfBlockedTurnRightElseMove') {
          // ✅ 一条指令跑完整个 while+if/else
          await this.execLoopBranchUntilSuccess()
          await this.sleep(80)
...
.orbtn{ background:linear-gradient(90deg,#ffe9f2 0%,#ffd1e6 100%); color:#b9165c; }
.until{ background:linear-gradient(90deg,#eef2ff 0%,#dbe9ff 100%); color:#4f6cff; }
.loopif{ background:linear-gradient(90deg,#fff1d6 0%,#ffe2b8 100%); color:#9a5b00; }
