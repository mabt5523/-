<template>
  <view class="page">
    <view class="header">
      <text class="title">第 41 关：拿钥匙开门 🔑🚪</text>
      <text class="tip">学习点：变量 keys（钥匙数量）</text>
    </view>

    <!-- ✅ 变量面板 -->
    <view class="var-panel">
      <text class="var-title">变量面板</text>
      <view class="var-row">
        <text class="var-k">keys</text>
        <text class="var-v">= {{ keys }}/3</text>
        <text class="var-note" :class="{ ok: keys>=3 }">{{ keys>=3 ? '门已可开' : '还差钥匙' }}</text>
      </view>
    </view>
    ...
    <view class="game-area">
      ...
      <!-- 钥匙 -->
      <view v-for="(k, idx) in keyCells" :key="'k'+idx" class="key" :style="toPxStyle(k.r, k.c)">
        🔑
      </view>

      <!-- 门：没开是🚪，开了是🚪✨（你也可以换成🟩之类） -->
      <view class="door" :style="toPxStyle(door.r, door.c)">
        <text>{{ doorOpen ? '🚪✨' : '🚪' }}</text>
      </view>
    ...
    <view class="panel">
      <view class="panel-title">
        指令序列（点可删除）
        <text class="hint">提示：先收集3把🔑再去🚪</text>
      </view>
      ...
      <view class="rule">
        <text>规则：</text>
        <text>
          走到🔑会自动 keys+1；门🚪需要 keys≥3 才能通过；
          红灯❗不能踩到；撞墙🧱或越界会停止。
        </text>
      </view>
      ...
      <view class="hintbox">
        ...
        <text>变量（Variable）= 程序用来“记住信息”的盒子，比如钥匙数量 keys。</text>
      </view>
    </view>
    ...
    <view v-if="success" class="success-popup" @touchmove.stop.prevent>
      <view class="success-content">
        <text class="success-text">🎉 通关成功！你会用变量啦</text>
    ...
    <view v-if="fail" class="fail-popup" @touchmove.stop.prevent>
      <view class="fail-content">
        <text class="fail-text">😵 失败了！</text>
        <text class="fail-desc">{{ failMsg }}</text>
...
export default {
  ...
  methods: {
    ...
    isDoor(cell) {
      return cell.r === this.door.r && cell.c === this.door.c
    },

    isKey(cell) {
      return this.keyCells.findIndex(k => k.r === cell.r && k.c === cell.c)
    },

    checkSuccessByDoor(cell) {
      return cell.r === this.door.r && cell.c === this.door.c && this.keys >= 3
    },


    collectKeyAt(cell) {
      const idx = this.isKey(cell)
      if (idx >= 0) {
        this.keyCells.splice(idx, 1)   // 从地图移除
        this.keys += 1                 // ✅ 变量+1
        uni.vibrateShort({ type: 'light' })
        uni.showToast({ title: `拿到钥匙！keys=${this.keys}/3`, icon: 'none' })
        if (this.keys >= 3 && !this.doorOpen) {
          this.doorOpen = true
          uni.showToast({ title: '钥匙够了！门可以打开🚪', icon: 'none' })
        }
      }
    },
    ...
    async run() {
      ...
      this.keys = 0
      this.doorOpen = false
      this.keyCells = this.keyCellsStart.map(x => ({ ...x }))
        ...
        if (cmd.type === 'turnLeft') {
          ...
          if (this.isDoor(next)) {
            if (this.keys >= 3) {
              ...
              this.fail = true
              this.failMsg = `门打不开：需要 keys≥3（现在 keys=${this.keys}）`
              break
            }
          }
...
.header{ text-align:center; color:#fff; position:relative; margin-bottom:18rpx; }
.back-btn{
...
.robot-wrap,.target,.danger,.block,.key,.door{
  position: absolute;
...
.key{ font-size: 38px; z-index: 1; filter: drop-shadow(0 8rpx 14rpx rgba(255,193,7,0.25)); }
.door{ font-size: 40px; z-index: 1; filter: drop-shadow(0 10rpx 18rpx rgba(0,0,0,0.18)); }
...
.popup-actions, .fail-actions{ display:flex; gap: 18rpx; justify-content:center; }
.popup-btn, .fail-btn{
  flex: 1;
  padding: 18rpx 0;
  border-radius: 26rpx;
...
.fail-btn{ width: 60%; background: linear-gradient(90deg,#ff9f43 0%,#ff6b6b 100%); color:#fff; }
</style>
